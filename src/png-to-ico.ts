/*
** Original repository https://github.com/steambap/png-to-ico
**
** This code is modification above png-to-ico package.
** Changes:
** - add ESM module
** - add typescript
** - allow any icon size
...
import type {default as Jimp} from 'jimp'

export async function pngToIco(file: Buffer): Promise<Buffer> {
	const image = await jimp.read(file)
	return imagesToIco([image])
}

function imagesToIco(images: Jimp[]) {
...
function getDir(img: Jimp, offset: number) {
...
function getBmpInfoHeader(img: Jimp) {
...
function getRowStride(width: number): number {
	if (width % 32 === 0) {
		return width / 8
	} else {
